...
def classify(image_dir, save_dir):

    img_list = os.listdir(image_dir)

    output_file = open(os.path.join(save_dir, 'log.txt'), 'w+')
    ...
    for img in img_list:
        ...
        label_lines = [line.rstrip() for line
                           in tf.gfile.GFile("retrained_labels-4_4N_5_7.txt")]
        ...
        with tf.gfile.FastGFile("retrained_graph-4_4N_5_7.pb", 'rb') as f:
        ...
        with tf.Session() as sess:
            ...
            print('Predictions for image: {} '.format(img))
            output_file.write('Predictions for image: {}\n'.format(img))
            # output
            for node_id in top_k:
                human_string = label_lines[node_id]
                score = predictions[0][node_id]
                print('%s (score = %.5f)' % (human_string, score))
                output_file.write('%s (score = %.5f)\n' % (human_string, score))
